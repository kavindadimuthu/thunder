...
The `ThunderIDProvider` must be used at the root of your Vue application to ensure that all components have access to authentication features. After installing the [`ThunderIDPlugin`](/docs/next/sdks/vue/apis/plugin/thunderid-plugin), the component is registered globally and can be used without explicit imports.
